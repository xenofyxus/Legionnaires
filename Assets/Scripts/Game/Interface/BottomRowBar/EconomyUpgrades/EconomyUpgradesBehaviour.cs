...
namespace Game.Interface.BottomRowBar.EconomyUpgrades
{
	public class EconomyUpgradesBehaviour : MonoBehaviour
	{
		...
		{
			if (GetComponent<KingUpgrades.UpgradesPanelBehaviour>().Disabling)
			{
				GetComponent<KingUpgrades.UpgradesPanelBehaviour>().Enable();
			}
			else
			{
				GetComponent<KingUpgrades.UpgradesPanelBehaviour>().Disable();
			}
		}
	}
}